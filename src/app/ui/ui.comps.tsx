...
import { Microsft } from "./microsoft";
import { useLocalContext } from "../context/LocalContext";

export default class UiCompsPage {
        ...
        return (
            <div className="flex flex-col justify-center items-center border border-slate-400 rounded-2xl my-2 px-2 py-4 " >
                ...
                <ul className="flex flex-col justify-center items-center p-3">
                    <this.ListHm color='gray' logo={faSteam} click={() => { pre.ButtonBoolean({ type: "openSteam" }) }} title="Steam" />
                    <this.ListHm color='skyblue' logo={faMicrosoft} click={() => { pre.ButtonBoolean({ type: "openMicrosoft" }) }} title="Microsft" />
                </ul>
                {pre.booleanState.steam && <Steam />}
                {pre.booleanState.microsoft && <Microsft />}
            </div>
        )
    }
    ListHm: React.FC<ListProps> = ({ ...pre }) => {
        return (
    ...
    ResultComp: React.FC<{ State: ecuationProp }> = ({ State }) => {
    ...
    OpTemplate: React.FC<TemplateProp> = ({ ...pre }) => {
        ...
        return (
            <div className=" flex flex-col justify-center items-center border border-slate-400 p-10 rounded-xl">
                ...
                {pre.result !== null && <this.ResultComp State={pre.factor} />}
                {booleanState.error && <this.ErrorComp />}
            </div>
        )
    }
}